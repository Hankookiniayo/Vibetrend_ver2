  ...
  const posts = [
    {
      ...
      excerpt: '유튜브 알고리즘의 변화와 2025년 상반기를 관통할 핵심 유튜브 트렌드 10가지를 정리했습니다.',
      date: '2025.02.01',
      readTime: '8 min read',
      author: '이OO 대표'
    },
    {
      category: '유튜브 가이드',
      title: 'Shorts vs Long-form: 유튜브 알고리즘의 비밀',
      excerpt: '단순한 조회수 비교를 넘어, 쇼츠와 본영상이 서로의 알고리즘에 미치는 영향과 최적의 업로드 주기를 분석합니다.',
      date: '2025.01.28',
    ...
    {
      category: '성공 전략',
      title: '클릭률(CTR)을 200% 높이는 유튜브 썸네일 트렌드',
      excerpt: '최근 유튜브에서 유행하는 썸네일 디자인 패턴과 클릭을 유도하는 심리학적 요소들을 분석했습니다.',
      date: '2025.01.20',
    ...
    <div className="pt-40 pb-20">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-24">
          <h1 className="text-5xl font-black mb-6">유튜브 인텔리전스 허브</h1>
          <p className="text-zinc-400 text-lg">채널 성장을 가속화하는 최신 유튜브 리포트와 분석 가이드를 만나보세요.</p>
        </div>
            ...
            {posts.map((post, i) => (
              <div key={i} className="glass p-10 rounded-[40px] group cursor-pointer hover:border-white/10 transition-all flex flex-col md:flex-row gap-10">
                ...
                <div className="md:w-2/3 flex flex-col justify-center">
                  ...
                  <h3 className="text-3xl font-black mb-4 group-hover:text-red-500 transition-colors">{post.title}</h3>
          ...
          <div className="space-y-12">
            <div>
              <h4 className="font-bold mb-6 flex items-center gap-2 text-zinc-400 uppercase text-xs tracking-widest">
                <Search size={14} /> Search YouTube Guides
              </h4>
            ...
            <div>
              ...
              <ul className="space-y-3">
                {['전체보기', '유튜브 분석', '알고리즘 가이드', '썸네일 전략', '인터뷰', '주간 유튜브 리포트'].map((c, i) => (
                  <li key={i} className={`text-sm font-bold cursor-pointer hover:text-white transition-colors ${i === 0 ? 'text-white' : 'text-zinc-500'}`}>{c}</li>
                ))}
              </ul>
            </div>

            <div className="glass p-8 rounded-3xl glow-blue border-white/5">
              <h4 className="text-xl font-black mb-4">지금 바로 채널을<br />성장시켜보세요</h4>
              <p className="text-xs text-zinc-500 mb-8 leading-relaxed">유튜브 크리에이터를 위한 유일한 데이터 인텔리전스</p>
